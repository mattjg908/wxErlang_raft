...
init([]) ->
    ...
    lists:foreach(
      fun(B) -> wxSizer:add(BtnGrid, B, [{flag, ?wxEXPAND}, {proportion, 1}]) end,
      [BtnRun, BtnStep, BtnClient, BtnTimeout, BtnStop, BtnResume, BtnRestart, BtnAlign, BtnSpread]),

    wxSizer:add(Sizer, BtnGrid, [{flag, ?wxEXPAND bor ?wxLEFT bor ?wxRIGHT bor ?wxBOTTOM}, {border, 8}]),

    wxSizer:add(Sizer, wxStaticText:new(Sidebar, ?wxID_ANY, "Speed"), [{flag, ?wxLEFT bor ?wxRIGHT}, {border, 8}]),
    Speed = wxSlider:new(Sidebar, ?ID_SPEED, 20, 1, 200, [{style, ?wxSL_HORIZONTAL bor ?wxSL_VALUE_LABEL}]),
    ...
    lists:foreach(
      fun({Obj, Ev}) -> wxEvtHandler:connect(Obj, Ev) end,
...
handle_event(#wx{id = ?ID_RUN, event = #wxCommand{type = command_button_clicked}}, St) ->
    ...
    Label = case Running of true -> "Pause"; false -> "Run" end,
...
handle_event(#wx{event = #wxMouse{type = left_down, x = X, y = Y}}, St) ->
...
refresh_info(St) ->
    #model{time = T, servers = Ss, messages = Ms} = St#st.model,
    ...
    LeaderTxt =
        case Leader0 of
            none -> "none";
            {LId, LTerm} -> lists:flatten(io_lib:format("S~p (term ~p)", [LId, LTerm]))
        end,
    Txt = io_lib:format(
        "time: ~p us~nleader: ~s~nmessages_in_flight: ~p~n~nselected: S~p~nstate: ~p~nterm: ~p~nvoted_for: ~p~nlog_len: ~p~ncommit_index: ~p~nelection_due_in: ~p us~n~nservers:~n~s",
        [T, LeaderTxt, length(Ms),
         Sel, SelS#server.state, SelS#server.term, SelS#server.voted_for,
         length(SelS#server.log), SelS#server.commit_index,
...
to_text(S) when is_binary(S) -> binary_to_list(S);
to_text(S) -> io_lib:format("~ts", [S]).
...
dc_setTextForeground(DC, Col) ->
    case erlang:function_exported(wxDC, setTextForeground, 2) of
        true -> _ = (catch wxDC:setTextForeground(DC, Col));
        false -> warn_once(setTextForeground, 2)
    end,
    ok.
    ...
    case erlang:function_exported(wxDC, drawLine, 5) of
        true ->
            case catch wxDC:drawLine(DC, X1, Y1, X2, Y2) of
                {'EXIT', _} -> dc_drawLine_tuple(DC, X1, Y1, X2, Y2);
                _ -> ok
            end;
        false ->
...
dc_drawLine_tuple(DC, X1, Y1, X2, Y2) ->
    case erlang:function_exported(wxDC, drawLine, 3) of
        true -> _ = (catch wxDC:drawLine(DC, {X1, Y1}, {X2, Y2}));
        false -> warn_once(drawLine, 3)
    end,
    ok.
    ...
    case erlang:function_exported(wxDC, drawCircle, 4) of
        true ->
            case catch wxDC:drawCircle(DC, X, Y, R) of
                {'EXIT', _} -> dc_drawCircle_tuple(DC, X, Y, R);
                _ -> ok
            end;
        false ->
...
dc_drawCircle_tuple(DC, X, Y, R) ->
    case erlang:function_exported(wxDC, drawCircle, 3) of
        true -> _ = (catch wxDC:drawCircle(DC, {X, Y}, R));
        false -> warn_once(drawCircle, 3)
    end,
    ok.
    ...
    case erlang:function_exported(wxDC, drawRectangle, 5) of
        true ->
            case catch wxDC:drawRectangle(DC, X, Y, W, H) of
                {'EXIT', _} -> dc_drawRectangle_tuple(DC, X, Y, W, H);
                _ -> ok
            end;
        false ->
...
dc_drawRectangle_tuple(DC, X, Y, W, H) ->
    case erlang:function_exported(wxDC, drawRectangle, 3) of
        true -> _ = (catch wxDC:drawRectangle(DC, {X, Y, W, H}));
        false -> warn_once(drawRectangle, 3)
    end,
    ok.
    ...
    case erlang:function_exported(wxDC, drawText, 4) of
        true ->
            case catch wxDC:drawText(DC, Str, X, Y) of
                {'EXIT', _} -> dc_drawText_tuple(DC, Str, X, Y);
                _ -> ok
            end;
        false ->
...
dc_drawText_tuple(DC, Str, X, Y) ->
    case erlang:function_exported(wxDC, drawText, 3) of
        true -> _ = (catch wxDC:drawText(DC, Str, {X, Y}));
        false -> warn_once(drawText, 3)
    end,
    ok.
...
draw_servers(DC, #model{time = Now, servers = Ss}, Positions, Selected) ->
    ...
    lists:foreach(
      fun(S) ->
          ...
          State =
              case State0 of
                  leader when VisibleLeader =:= Id -> leader;
                  leader -> follower;
                  Other -> Other
              end,
...
role_short(follower) -> "F";
role_short(stopped) -> "X";
role_short(_) -> "?".
