...
new() ->
    seed_rand(),
    Servers = [server(I, peers(I)) || I <- lists:seq(1, ?NUM_SERVERS)],
    #model{time = 0,
           servers = Servers,
           messages = []}.
...
spread_timers(Model0) ->
    ...
    Timers =
        lists:sort([S#server.election_alarm
                    || S <- Model0#model.servers,
                       S#server.election_alarm > Time,
                       S#server.election_alarm < ?INF]),
    case Timers of
        %% If multiple servers have the exact same earliest election alarm,
        %% the original "push timers within (T0, T0 + MAX_RPC_LATENCY)" rule
        %% doesn't move them because they are not strictly greater than T0.
        %% Break ties deterministically by pushing all-but-one forward.
        [T0, T0 | _] ->
            %% Keep the smallest id at T0; push the others by i*MAX_RPC_LATENCY.
            Ss0 = [S
                   || S <- Model0#model.servers,
                      S#server.election_alarm =:= T0,
                      S#server.election_alarm > Time,
                      S#server.election_alarm < ?INF],
            %% Stable order for deterministic spreading.
            Ss1 = lists:keysort(#server.id, Ss0),
            {_Keep, Move} =
                case Ss1 of
                    [] ->
                        {undefined, []};
            ...
            %% Avoid relying on lists:zipwith/3 for older OTP releases.
            NMove = length(Move),
            Idxs =
                case NMove of
                    0 ->
            ...
            IdToNewAlarm =
                [{S#server.id, T0 + I * ?MAX_RPC_LATENCY} || {S, I} <- lists:zip(Move, Idxs)],
            Servers1 =
                [case lists:keyfind(S#server.id, 1, IdToNewAlarm) of
                     {_, NewA} ->
                         Tdur0 = NewA - S#server.election_start,
                         Tdur1 = erlang:max(1, Tdur0),
                         S#server{election_timeout = Tdur1, election_alarm = NewA};
                     false ->
                         S
                 end
                 || S <- Model0#model.servers],
...
apply_rules(M0, S0) ->
    ...
    lists:foldl(fun(Peer, {SAcc, MAcc}) ->
...
start_new_election(M = #model{time = Time}, S) ->
    case (S#server.state =:= follower orelse S#server.state =:= candidate)
         andalso S#server.election_alarm =< Time
    of
        true ->
            Peers = S#server.peers,
            Alarm = make_election_alarm(Time),
            S1 = S#server{election_start = Time,
...
send_append_entries(M = #model{time = Time}, S, Peer) ->
    case S#server.state =:= leader of
        ...
        true ->
            ...
            ShouldSend = HeartbeatDue =< Time orelse NextIndex =< LogLen andalso RpcDue =< Time,
...
handle_request_vote_request(M0, S0, Req) ->
    ...
    Granted =
        case S1#server.term =:= maps:get(term, Req) of
            ...
            true ->
                ...
                UpToDate =
                    ReqLastTerm > MyLastTerm
                    orelse ReqLastTerm =:= MyLastTerm andalso ReqLastIdx >= MyLastIdx,
    ...
    S2 = case Granted of
             true ->
                 Now = M1#model.time,
                 Alarm = make_election_alarm(Now),
                 S1#server{voted_for = maps:get(from, Req),
                           election_start = Now,
                           election_timeout = Alarm - Now,
                           election_alarm = Alarm};
             false ->
    ...
    {Success, MatchIndex, S2} =
        case TermOk of
            ...
            true ->
                ...
                LogOk =
                    PrevIndex =:= 0
                    orelse PrevIndex =< length(Sx#server.log)
                           andalso log_term(Sx#server.log, PrevIndex) =:= PrevTerm,
                case LogOk of
                    false ->
                        {false, 0, Sx};
                    true ->
            ...
            case maps:get(success, Reply) of
                true ->
                    ...
                    Match2 =
                        maps:put(From,
                                 erlang:max(
                                     maps:get(From, S1#server.match_index), MI),
                                 S1#server.match_index),
                    Next2 = maps:put(From, MI + 1, S1#server.next_index),
                    {S1#server{match_index = Match2,
                               next_index = Next2,
                               rpc_due = RpcDue2},
                     M1};
                false ->
...
append_entries(Log0, PrevIndex, Entries) ->
    ...
    {Log1, LastIdx} =
        lists:foldl(fun(Entry, {LogAcc, IdxAcc}) ->
                       Idx = IdxAcc + 1,
